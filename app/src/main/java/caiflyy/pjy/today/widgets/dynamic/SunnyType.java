...
import static caiflyy.pjy.today.utils.TimeUtilsKt.getTimeDiffPercent;
...
public class SunnyType extends BaseWeatherType {
    ...
    public SunnyType(Context context, ShortWeatherInfo info) {
        ...
        currentSunPosition = getTimeDiffPercent(info.getSunrise(), info.getSunset());
        currentMoonPosition = getTimeDiffPercent(info.getMoonrise(), info.getMoonset());
        if (currentSunPosition >= 0 && currentSunPosition <= 1) {
            setColor(colorDay);
            boat = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.img_weather_dynamic_ic_boat_day);
        } else {
            setColor(colorNight);
            boat = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.img_weather_dynamic_ic_boat_night);
        }

        cloud = BitmapFactory.decodeResource(getContext().getResources(), R.drawable.img_weather_dynamic_ic_cloud);
